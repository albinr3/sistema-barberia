create index if not exists barbers_active_name_idx
  on public.barbers (is_active, display_name);

create index if not exists services_active_sort_order_idx
  on public.services (is_active, sort_order, name);

create index if not exists barber_services_service_active_idx
  on public.barber_services (service_id, is_active);

create index if not exists availability_rules_barber_day_active_idx
  on public.availability_rules (barber_id, day_of_week, is_active);

create index if not exists availability_exceptions_barber_date_idx
  on public.availability_exceptions (barber_id, exception_date);

create index if not exists appointments_barber_status_time_idx
  on public.appointments (barber_id, status, starts_at, ends_at);
