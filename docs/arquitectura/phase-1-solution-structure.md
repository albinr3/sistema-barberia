...
La administracion local puede crear, editar, desactivar y eliminar barberos sin historial. Debe tratar `is_active` como atributo administrativo distinto de `BarberState`, porque Fase 2/3 sincronizaran ese atributo con la base cloud.
...
- Atributos centrales del barbero necesarios para sincronizacion futura, como imagen opcional e `is_active`.
