...
- El kiosko imprime tickets reales mediante `WindowsGraphicsKioskTicketPrinter`, que dibuja una pagina Windows/GDI optimizada para rollo termico 80mm en la impresora predeterminada.
...
- No se envian comandos ESC/POS desde kiosko ni autocaja; el modelo/driver del piloto puede imprimir esos bytes como letras/numeros o quedarse procesando durante varios minutos.
- El equipo de kiosko y autocaja debe tener una impresora termica 80mm configurada como predeterminada en Windows; si no existe o el driver rechaza trabajos graficos, el boton de imprimir muestra error y no simula exito.
...
- La autocaja imprime recibos, reimpresiones y reporte diario mediante `WindowsGraphicsCashBoxReceiptPrinter`, usando Windows/GDI sobre la impresora predeterminada 80mm. El cash drawer sigue simulado; el escaner QR queda como contrato disponible para integrar cuando se conecte el flujo de lectura fisica.
