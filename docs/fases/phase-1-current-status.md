...
- Administracion local: la cola activa permite cancelar tickets `waiting`, `called` o `in_service`; al cancelar un ticket con barbero asignado, el barbero activo vuelve a `available`, se intenta asignar automaticamente el siguiente ticket `waiting` compatible y se registra auditoria.
...
- Ultimo paso completado: `Unificacion de TurnState.Assigned en TurnState.Called`.
- Ultima verificacion ejecutada: `dotnet test BarberiaSystem.sln --no-restore -m:1 -v:minimal -p:UseSharedCompilation=false` y `dotnet build src/desktop/Barberia.Desktop/Barberia.Desktop.csproj --no-restore -m:1 -v:minimal -p:UseSharedCompilation=false "-p:BaseOutputPath=c:\Users\Albin Rodriguez\Documents\sistema-barberia\.build-validation\"` en 2026-06-07; resultado correcto, 68 pruebas superadas y Desktop compilado sin advertencias en salida temporal.
